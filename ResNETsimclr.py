...
class SimCLRModel(nn.Module):
    def __init__(self, projectionHeadDimension):
        super().__init__()
        self.backbone_model = models.resnet50(pretrained=False, 
                            num_classes=projectionHeadDimension)
        final_dimension_length = self.backbone_model.fc.in_features
        self.backbone_model.fc = nn.Sequential(
            nn.Linear(final_dimension_length, final_dimension_length),
            nn.ReLU(), 
            self.backbone_model.fc
            )
